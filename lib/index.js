...
class LruCache {
	constructor (capacity, cb) {
		...
		this.links = new CircularDoublyLinkedList();
		this.cache = new Map();
		this.maxcap = capacity || 10;
		this.cb = cb;
	}
	...
	put (key, value) {
		...
		this.cache.set(
			key,
			this.links.pushFront(Entry(key, value))
		);

		// evict
		if (this.links.size() > this.maxcap) {
			const lru = this.links.pop();
	...
	resize (cap) {
		...
		let diff = this.links.size() - cap;

		if (diff < 0) diff = 0;

		for (let i = 0; i < diff; i++) {
			const lru = this.links.tail();
			if (lru) {
				this.links.remove(lru);
				this.cache.delete(lru.value.key);
